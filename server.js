...
// Native macOS speech control and caching system
...
class SpeechController {
  ...
  stopCurrent() {
    ...
    if (os.platform() === 'darwin') {
      try { execSync('killall say 2>/dev/null || true'); } catch (e) {}
    }
  }
  ...
  speakLocally(text) {
    ...
    function attempt(voice) {
      const args = voice ? ['-v', voice] : [];
      self.process = spawn('/usr/bin/say', args);
      self.process.stdin.write(text);
      self.process.stdin.end();

      self.process.on('exit', (code) => {
        if (code !== 0 && voiceIndex < voices.length) {
          attempt(voices[voiceIndex++]);
        } else {
          self.process = null;
        }
      });
...
const localSpeaker = new SpeechController();
...
app.all('/api/speak', (req, res) => {
  ...
  if (os.platform() !== 'darwin') {
    return res.status(400).json({ success: false, message: 'Speech only supported on macOS host.' });
  }

  const hash = crypto.createHash('md5').update(clean).digest('hex');
  const cacheDir = path.join(os.tmpdir(), 'jenny-speak-cache');
  if (!fs.existsSync(cacheDir)) {
    try { fs.mkdirSync(cacheDir, { recursive: true }); } catch {}
  }
  const cacheFile = path.join(cacheDir, `${hash}.aiff`);
  ...
  if (!fs.existsSync(cacheFile)) {
    ...
    for (const voice of voices) {
      try {
        execSync(`say -v "${voice}" -o "${cacheFile}" "${clean.replace(/"/g, '\\"')}"`);
        success = true;
        break;
      } catch (e) {
        console.warn(`[Speak API] voice ${voice} failed:`, e.message);
      }
    }
    if (!success) {
      try {
        execSync(`say -o "${cacheFile}" "${clean.replace(/"/g, '\\"')}"`);
      } catch (e) {
        console.error('[Speak API] Fallback say failed:', e);
        return res.status(500).json({ success: false, message: 'Speech generation failed' });
      }
    }
  }

  localSpeaker.stopCurrent();
  res.setHeader('Content-Type', 'audio/x-aiff');
  res.sendFile(cacheFile);
});

app.post('/api/speak/fallback', (req, res) => {
  const { text } = req.body;
  if (text) {
    localSpeaker.speakLocally(text);
  }
...
// Endpoint to get basic macOS system status (fully cached for 0ms latency)
app.get('/api/system', (req, res) => {
  ...
  res.json({
    success: true,
    data: {
      battery: cachedBattery.level !== undefined ? {
        percent: cachedBattery.level,
        state: cachedBattery.charging ? 'charging' : 'discharging'
      } : 'Unknown',
      uptime: cachedSystemStatus.uptime ? cachedSystemStatus.uptime + " seconds" : 'Unknown',
      volume: cachedVolume,
      brightness: cachedBrightness,
      ip: cachedIp,
      os: 'macOS',
      cpu: cachedSystemStatus.cpu.usage,
      ram: cachedSystemStatus.ram.usage
    }
  });
});
...
let cachedBattery = { level: 100, charging: true };
let cachedDisk = { usage: 35, free: '120GB' };
let cachedNet = { usage: 0, speed: '0 KB/s' };
let cachedVolume = 50;
let cachedBrightness = 0.8;
let cachedIp = 'Unknown';


function updateHeavySystemTelemetry() {
  // 1. Battery
  exec('pmset -g batt', { timeout: 3000 }, (err, stdout) => {
    if (!err && stdout) {
      const match = stdout.match(/(\d+)%/);
      let level = cachedBattery.level;
  ...
  // 2. Disk
  exec('df -h /', { timeout: 3000 }, (errDisk, stdoutDisk) => {
    if (!errDisk && stdoutDisk) {
      ...
      if (lines.length > 1) {
        ...
        if (parts.length >= 5) {
          cachedDisk = {
            free: parts[3],
            usage: parseInt(parts[4].replace('%', ''), 10) || 35
          };
        }
      }
    }
  ...
  // 3. Network bytes
  exec('netstat -ib', { timeout: 3000 }, (errNet, stdoutNet) => {
    let totalBytes = 0;
    if (!errNet && stdoutNet) {
    ...
    cachedNet = { usage: netUsagePct, speed: netSpeedStr };
  });

  // 4. Volume
  exec("osascript -e 'output volume of (get volume settings)'", { timeout: 2000 }, (errVol, stdoutVol) => {
    if (!errVol && stdoutVol) {
      cachedVolume = parseInt(stdoutVol.trim(), 10) || 50;
    }
  });
  ...
  exec(pythonBrightnessCmd, { timeout: 2000 }, (errBright, stdoutBright) => {
    if (!errBright && stdoutBright) {
      cachedBrightness = parseFloat(stdoutBright.trim()) || 0.8;
    }
  });
}

// Update heavy stats every 10s
setInterval(updateHeavySystemTelemetry, 10000);
setTimeout(updateHeavySystemTelemetry, 1500);
...
function updateSystemStatusTelemetry() {
  ...
  cachedSystemStatus = {
    ...
    battery: { level: cachedBattery.level, charging: cachedBattery.charging },
    disk: { usage: cachedDisk.usage, free: cachedDisk.free },
    net: { usage: cachedNet.usage, speed: cachedNet.speed },
    uptime: Math.round(os.uptime()),
    hostname: os.hostname(),
    platform: os.platform()
  };
...
// Start CPU/RAM background updates every 3s (zero shell overhead)
updateSystemStatusTelemetry();
setInterval(updateSystemStatusTelemetry, 3000);
...
function autoOpenBrowser(customUrl) {
  const url = customUrl || `http://localhost:${PORT}`;
  const platform = os.platform();

  console.log(`[JENNY] Launching assistant interface...`);
...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.log(`[JENNY] Port ${PORT} is already in use. Opening active session...`);
    autoOpenBrowser(`http://localhost:${PORT}/?alert=already_running`);
    setTimeout(() => {
      process.exit(0);
    }, 1000);
  } else {
