...
# alias maps to a canonical phrase the command router already understands, so
# "close the browser" == "kill firefox" == "exit the web" all work. This is a
# BROAD INTERNAL CATALOG (~500+ mappings, not displayed anywhere in the UI):
# every alias expands BEFORE routing so free-form, case-insensitive phrasing
# resolves offline and deterministically instead of depending on the LLM.
# Canonical targets are kept verb-preserving ("open X" / "close X") or set to
# exact branch trigger phrases so the router's regex branches always fire.
COMMAND_SYNONYMS = {
    # ---- launch / open (verb-preserving) ----
    "launch": "open", "start up": "open", "boot up": "open", "fire up": "open",
    "bring up": "open", "pop open": "open", "run program": "open", "run app": "open",
    "load up": "open", "start the app": "open", "get the app going": "open",
    "start the program": "open", "kick off": "open", "start app": "open",
    "crank up": "open", "spin up": "open", "open up": "open",
    "open app": "open", "open the app": "open", "open the program": "open",
    "start an app": "open", "start an application": "open", "power up the app": "open",
    "get the program running": "open", "get the app running": "open",
    # ---- close / quit (verb-preserving) ----
    "kill": "close", "shut down app": "close", "close down": "close", "exit app": "close",
    "quit the app": "close", "quit app": "close", "close out": "close",
    "terminate the app": "close", "shut the app": "close", "end the program": "close",
    "close the program": "close", "exit the program": "close", "exit out of": "close",
    "get rid of the app": "close", "dismiss app": "close", "bounce the app": "close",
    "close the app down": "close", "quit the application": "close",
    "end the application": "close", "shut the program": "close", "kill the program": "close",
    # ---- screenshot ----
    "capture screen": "screenshot", "take a screenshot": "screenshot", "snapshot": "screenshot",
    "cap the screen": "screenshot", "screen grab": "screenshot", "print screen": "screenshot",
    "screenshot the screen": "screenshot", "take a pic of the screen": "screenshot",
    "grab the screen": "screenshot", "capture the display": "screenshot", "screeny": "screenshot",
    "screenshot the desktop": "screenshot", "screen capture": "screenshot",
    "grab a shot": "screenshot", "screen dump": "screenshot", "snap the screen": "screenshot",
    "take the screen": "screenshot", "capture the screen": "screenshot",
    # ---- lock ----
    "secure my pc": "lock pc", "lock workstation": "lock pc", "lock the computer": "lock pc",
    "lock my pc": "lock pc", "lock the pc": "lock pc", "lock the screen": "lock pc",
    "lock my computer": "lock pc", "lock down the pc": "lock pc", "lock the laptop": "lock pc",
    "secure the computer": "lock pc", "lock windows": "lock pc", "lock my laptop": "lock pc",
    "put a lock on the pc": "lock pc", "lock the machine": "lock pc", "lock down": "lock pc",
    "secure the machine": "lock pc", "lock the workstation": "lock pc",
    "freeze the screen": "lock pc", "secure the screen": "lock pc",
    # ---- empty trash / recycle bin ----
    "empty bin": "empty trash", "clear the trash": "empty trash", "dump recycle bin": "empty trash",
    "empty the recycle bin": "empty trash", "empty trash can": "empty trash",
    "clear recycle bin": "empty trash", "empty the bin": "empty trash", "dump trash": "empty trash",
    "trash it all": "empty trash", "clear the bin": "empty trash", "clean the trash": "empty trash",
    "empty the recycling bin": "empty trash", "clear the garbage": "empty trash",
    "empty the trash bin": "empty trash", "clear out the bin": "empty trash",
    # ---- power / shutdown / restart / sleep ----
    "power off": "shutdown", "power down": "shutdown", "switch off pc": "shutdown",
    "turn off the computer": "shutdown", "turn off pc": "shutdown", "turn the pc off": "shutdown",
    "shut the pc down": "shutdown", "shut down the computer": "shutdown",
    "power the pc off": "shutdown", "switch the pc off": "shutdown", "power the computer down": "shutdown",
    "turn off the pc completely": "shutdown", "bye bye pc": "shutdown",
    "switch it off": "shutdown", "kill the power": "shutdown",
    "reboot the pc": "restart", "reset the pc": "restart", "reboot the computer": "restart",
    "restart the computer": "restart", "restart the pc": "restart", "reboot my machine": "restart",
    "restart the machine": "restart", "reboot now": "restart", "restart now": "restart",
    "restart my machine": "restart", "reboot the machine": "restart",
    "make pc sleep": "sleep", "sleep mode": "sleep", "put pc to sleep": "sleep",
    "put the pc to sleep": "sleep", "go to sleep pc": "sleep", "sleep the pc": "sleep",
    "put the computer to sleep": "sleep", "make the pc sleep": "sleep", "sleep now": "sleep",
    "put the laptop to sleep": "sleep", "go into sleep mode": "sleep", "suspend the pc": "sleep",
    "zombie the pc": "sleep", "hibernate the pc": "sleep", "take the pc to sleep": "sleep",
    "nap mode": "sleep", "standby mode": "sleep", "go to standby": "sleep",
    # ---- minimize ----
    "minimise all": "minimize all", "minimize everything": "minimize all", "show my desktop": "minimize all",
    "show the desktop": "minimize all", "minimize the windows": "minimize all",
    "minimise the windows": "minimize all", "clear the desktop": "minimize all",
    "minimize my windows": "minimize all", "reduce all windows": "minimize all",
    "show desktop icons": "minimize all", "get everything out of the way": "minimize all",
    "hide all windows": "minimize all", "minimize all windows": "minimize all",
    "shrink all windows": "minimize all", "put the windows away": "minimize all",
    # ---- terminal / cmd ----
    "terminal window": "open terminal", "open command prompt": "open terminal", "launch cmd": "open terminal",
    "open terminal window": "open terminal", "launch terminal": "open terminal", "run cmd": "open terminal",
    "start cmd": "open terminal", "open a terminal": "open terminal", "open command line": "open terminal",
    "open powershell": "open terminal", "start a terminal": "open terminal", "cli window": "open terminal",
    "show terminal": "open terminal", "bring up terminal": "open terminal", "open the terminal": "open terminal",
    "command prompt window": "open terminal", "open the command prompt": "open terminal",
    # ---- clipboard ----
    "whats on clipboard": "clipboard", "read clipboard": "clipboard", "clipboard contents": "clipboard",
    "what is on the clipboard": "clipboard", "show clipboard": "clipboard",
    "what's in my clipboard": "clipboard", "copy history": "clipboard", "read the clipboard": "clipboard",
    "clipboard status": "clipboard", "whats in my clipboard": "clipboard",
    "what was copied": "clipboard", "check the clipboard": "clipboard", "read my clipboard": "clipboard",
    "get my clipboard": "clipboard",
    # ---- volume ----
    "lower volume": "volume down", "turn volume down": "volume down", "quieter": "volume down",
    "lower the volume": "volume down", "make it quieter": "volume down", "volume down": "volume down",
    "reduce the volume": "volume down", "sound down": "volume down", "pull the volume down": "volume down",
    "make the volume lower": "volume down", "turn it down a bit": "volume down", "turn it down": "volume down",
    "decrease sound": "volume down", "lower sound": "volume down", "quieter please": "volume down",
    "make it less loud": "volume down", "cut the volume": "volume down", "lower the sound": "volume down",
    "soften the sound": "volume down", "reduce sound": "volume down",
    "softer": "volume down", "softer please": "volume down", "tone it down": "volume down",
    "not so loud": "volume down", "bring the volume down": "volume down",
    "raise volume": "volume up", "turn volume up": "volume up", "louder": "volume up",
    "raise the volume": "volume up", "make it louder": "volume up", "volume up": "volume up",
    "increase the volume": "volume up", "sound up": "volume up", "crank the volume": "volume up",
    "make the volume higher": "volume up", "turn it up a bit": "volume up", "turn it up": "volume up",
    "deafen me": "volume up", "increase sound": "volume up", "raise sound": "volume up",
    "pump up the volume": "volume up", "turn it up loud": "volume up", "boost the volume": "volume up",
    "boost volume": "volume up", "crank it up": "volume up", "amplify the sound": "volume up",
    "make it loud": "volume up", "max the volume": "volume up",
    "loud mode": "volume up", "get louder": "volume up", "turn the volume up": "volume up",
    # ---- mute / unmute ----
    "silence my pc": "mute", "mute audio": "mute", "turn off sound": "mute",
    "mute the volume": "mute", "mute my computer": "mute", "silence everything": "mute",
    "kill the sound": "mute", "mute all audio": "mute", "silent mode": "mute",
    "shut the audio up": "mute", "go quiet": "mute", "mute the computer": "mute",
    "mute the pc": "mute", "silence the pc": "mute", "mute sound": "mute",
    "quiet everything": "mute", "mute my pc": "mute", "no sound": "mute",
    "volume off": "mute", "kill audio": "mute", "quiet mode": "mute", "shush": "mute",
    "turn on sound": "unmute", "restore audio": "unmute", "unmute audio": "unmute",
    "unmute the volume": "unmute", "bring sound back": "unmute", "unmute my computer": "unmute",
    "turn sound back on": "unmute", "restore the sound": "unmute", "unmute the computer": "unmute",
    "unmute the pc": "unmute", "unmute sound": "unmute", "sound back on": "unmute",
    "enable audio": "unmute", "turn the sound on": "unmute", "unmute my pc": "unmute",
    ...
    "start the track": "media play", "hit play": "media play",
    "pause the song": "media pause", "pause the track": "media pause", "hold the music": "media pause",
    "freeze the playback": "media pause", "pause the playback": "media pause", "stop the song": "media pause",
    "pause the media": "media pause", "stop the track": "media pause", "quiet the playback": "media pause",
    "pause please": "media pause", "halt the track": "media pause", "freeze it": "media pause",
    "next track": "media next", "skip the song": "media next", "skip the track": "media next",
    "move to the next": "media next", "play the next one": "media next", "switch to the next song": "media next",
    "forward the track": "media next", "skip this song": "media next", "skip this": "media next",
    "skip ahead": "media next", "next please": "media next", "next up": "media next",
    "previous track": "media previous", "go back a song": "media previous", "play the last one": "media previous",
    "switch to the previous": "media previous", "go back a track": "media previous",
    "replay the last song": "media previous", "previous one": "media previous", "back up a track": "media previous",
    "push back the track": "media previous", "the previous track": "media previous",
    "previous please": "media previous", "go to the last track": "media previous",
    # ---- brightness ----
    "increase brightness": "brightness up", "brighter": "brightness up", "make screen brighter": "brightness up",
    "brighten the screen": "brightness up", "screen brighter": "brightness up", "turn up brightness": "brightness up",
    "brightness up": "brightness up", "make it brighter": "brightness up", "more bright": "brightness up",
    "raise the brightness": "brightness up", "increase the brightness": "brightness up", "lighten the screen": "brightness up",
    "max brightness": "brightness up", "full brightness": "brightness up", "lift the brightness": "brightness up",
    "screen more bright": "brightness up", "brighter please": "brightness up", "up the brightness": "brightness up",
    "brighten it": "brightness up", "boost brightness": "brightness up",
    "decrease brightness": "brightness down", "dimmer": "brightness down", "make screen dimmer": "brightness down",
    "dim the screen": "brightness down", "screen dimmer": "brightness down", "turn down brightness": "brightness down",
    "brightness down": "brightness down", "make it dimmer": "brightness down", "reduce the brightness": "brightness down",
    "decrease the brightness": "brightness down", "darken the screen": "brightness down", "lower the brightness": "brightness down",
    "bring the brightness down": "brightness down", "dimmer please": "brightness down", "reduce brightness": "brightness down",
    "drop the brightness": "brightness down", "darken the display": "brightness down", "screen less bright": "brightness down",
    "less bright": "brightness down", "pull the brightness down": "brightness down",
    "lower the screen light": "brightness down", "mood lighting": "brightness down",
    # ---- network / wifi ----
    "wifi status": "wifi", "network status": "wifi", "connected network": "wifi",
    "what wifi am i on": "wifi", "current wifi": "wifi", "wifi strength": "wifi",
    "what wifi'm i on": "wifi", "what wifi i'm on": "wifi", "what network'm i on": "wifi",
    "network name": "wifi", "which network": "wifi", "wifi info": "wifi",
    "my network": "wifi", "whats my wifi": "wifi", "wifi connected": "wifi",
    "am i online": "wifi", "network connection": "wifi", "connection status": "wifi",
    "show network": "wifi", "check wifi": "wifi", "what's my ip": "wifi", "ip address": "wifi",
    "whats my ip": "wifi", "my ip address": "wifi", "my ip": "wifi",
    "check my network status": "wifi", "what network am i on": "wifi",
    "what network'm i on": "wifi", "what's my network": "wifi", "whats my network": "wifi",
    "network info": "wifi", "my connection": "wifi", "internet check": "wifi",
    "what network am i on": "wifi", "is my internet up": "wifi", "wlan status": "wifi",
    # ---- files ----
    "file manager": "open files", "open files": "open files", "open file explorer": "open files",
    "open the files": "open files", "show my files": "open files", "file explorer": "open files",
    "open my documents": "open files", "show files": "open files", "open the file explorer": "open files",
    "browse my files": "open files", "explorer window": "open files", "open windows explorer": "open files",
    "show my folders": "open files", "files please": "open files", "my documents": "open files",
    "open my folder": "open files", "file browser": "open files", "take me to my files": "open files",
    "drop me in my files": "open files",
    # ---- note apps ----
    "open notes": "open notepad", "txtpad": "open notepad",
    "open notepad": "open notepad", "open the notepad": "open notepad", "start notepad": "open notepad",
    "launch notepad": "open notepad", "text pad": "open notepad", "notepad app": "open notepad",
    "open a note sheet": "open notepad", "text editor": "open notepad",
    "start a notepad": "open notepad", "open a scratchpad": "open notepad", "scratchpad": "open notepad",
    # ---- calculator ----
    "calculator app": "open calculator", "run calculator": "open calculator", "open calc": "open calculator",
    "open calculator": "open calculator", "start calculator": "open calculator", "launch calculator": "open calculator",
    "open the calculator": "open calculator", "calc time": "open calculator", "scientific calculator": "open calculator",
    "open a calculator": "open calculator", "do some math": "open calculator", "math app": "open calculator",
    # ---- browser / chrome ----
    "launch browser": "open chrome", "open web browser": "open chrome", "open internet": "open chrome",
    "open browser": "open chrome", "start the browser": "open chrome", "open the browser": "open chrome",
    "open the internet": "open chrome", "web browser": "open chrome", "start browser": "open chrome",
    "browser up": "open chrome", "open my browser": "open chrome", "internet browser": "open chrome",
    "bring up the browser": "open chrome", "fire up the browser": "open chrome",
    "start a browser": "open chrome", "get online": "open chrome", "go to chrome": "open chrome",
    "open a browser window": "open chrome",
    # ---- editor / vscode ----
    "open editor": "open vscode", "launch vscode": "open vscode", "code editor": "open vscode",
    "open vscode": "open vscode", "visual studio code": "open vscode", "open the editor": "open vscode",
    "start the code editor": "open vscode", "launch the code editor": "open vscode", "vs code": "open vscode",
    "open the code editor": "open vscode", "open code": "open vscode", "code editor up": "open vscode",
    "open visual studio code": "open vscode",
    "open my editor": "open vscode", "start the editor": "open vscode", "open the workspace": "open vscode",
    "open vs code": "open vscode", "start vs code": "open vscode", "launch vs code": "open vscode",
    # ---- spotify / music ----
    "open spotify": "open spotify", "launch music": "open spotify", "start spotify": "open spotify",
    "open the spotify": "open spotify", "launch spotify": "open spotify", "music app": "open spotify",
    "open the music app": "open spotify", "spotify up": "open spotify", "fire up spotify": "open spotify",
    "start the music app": "open spotify", "music please": "open spotify", "get spotify going": "open spotify",
    # ---- system hygiene ----
    "free ram": "purge ram", "clean memory": "purge ram", "clear the ram": "purge ram",
    "clean up memory": "purge ram", "flush ram": "purge ram", "empty ram": "purge ram",
    "free up memory": "purge ram", "clear out ram": "purge ram", "clean the ram": "purge ram",
    "optimize memory": "purge ram", "dump the ram": "purge ram", "release memory": "purge ram",
    "release ram": "purge ram", "tidy up memory": "purge ram", "clean the memory": "purge ram",
    "free system memory": "purge ram", "clean the memory cache": "purge ram",
    "ram cleanup": "purge ram", "boost performance": "purge ram", "speed up my pc": "purge ram",
    # ---- task manager ----
    "show process list": "task manager", "open processes": "task manager",
    "show the task manager": "task manager", "process explorer": "task manager",
    "show running processes": "task manager", "end task window": "task manager",
    "process list": "task manager", "show tasks list": "task manager", "list processes": "task manager",
    "what processes are running": "task manager",
    # ---- emails ----
    "read my email": "read emails", "check my emails": "read emails", "read my inbox": "read emails",
    "show my inbox": "read emails", "open my email": "read emails", "any new email": "read emails",
    "check email": "read emails", "read the inbox": "read emails", "show the emails": "read emails",
    "show my email": "read emails", "check my mail": "read emails", "read my mail": "read emails",
    "email check": "read emails", "inbox check": "read emails", "see my emails": "read emails",
    "get my mail": "read emails", "any mail for me": "read emails",
    # ---- display ----
    "wake the screen": "wake display", "turn on the display": "wake display",
    "wake the monitor": "wake display", "turn my display on": "wake display",
    "wake my screen": "wake display", "display on": "wake display", "monitor on": "wake display",
    "wake the laptop screen": "wake display", "turn the screen on": "wake display",
    "turn screen on": "wake display", "monitor wake": "wake display",
    # ---- timer ----
    "set a timer": "set timer", "make a timer": "set timer", "countdown for": "timer for",
    "give me a timer": "set timer", "start a countdown": "set timer", "remind me in": "set timer",
    "wake me in": "set timer", "alarm me": "set alarm", "set an alarm": "set alarm",
    "start a timer": "set timer", "count down from": "set timer", "alarm in": "set alarm",
    # ---- tasks / todo (verb-free canonicals keep the todo parser working) ----
    "show my tasks": "show todo", "display tasks": "show todo", "list my tasks": "show todo",
    "tasks please": "show todo", "show the tasks": "show todo", "what tasks": "show todo",
    "whats on my list": "show todo", "my to do list": "show todo", "show me my tasks": "show todo",
    "show the todo": "show todo", "what are my todos": "show todo", "my pending items": "show todo",
    "clear my tasks": "clear todo", "clear everything": "clear todo", "reset my todo": "clear todo",
    "remove everything": "clear todo", "wipe the todo list": "clear todo",
    "finish all tasks": "clear todo", "drop all tasks": "clear todo", "empty my todo": "clear todo",
    # ---- memory / vault ----
    "save that to my memory": "remember", "store that thought": "remember", "save this fact": "remember",
    "keep that in the vault": "remember", "memorize this": "remember", "write it to memory": "remember",
    "put it in the vault": "remember", "note that down": "remember", "log that": "remember",
    "keep a note": "remember", "save the detail": "remember", "remember that for me": "remember",
    "commit this to memory": "remember", "stock this in the vault": "remember",
    "file that away": "remember", "stash that": "remember", "squirrel this away": "remember",
    # ---- media app deep controls ----
    ...
    "what tabs do i have": "list tabs", "tabs please": "list tabs", "list my tabs": "list tabs",
    "make a new tab": "open new tab", "open a fresh tab": "open new tab", "create a tab": "open new tab",
    "start a new tab": "open new tab", "another tab": "open new tab",
    "reload this page": "refresh page", "refresh this page": "refresh page",
    "refresh the current page": "refresh page", "reload the page": "refresh page",
    "previous page please": "go back", "back a page": "go back",
    "go to the previous page": "go back", "forward a page": "go forward", "next page please": "go forward",
    "enter fullscreen": "fullscreen", "go full screen": "fullscreen", "full screen mode": "full screen",
    "make it fullscreen": "fullscreen",
    "close current tab": "close this tab", "close the tab i'm on": "close this tab",
    "kill this tab": "close this tab", "get rid of this tab": "close this tab",
    "shut this tab": "close this tab", "close this open tab": "close this tab",
    "open my bookmarks": "show bookmarks", "display bookmarks": "show bookmarks",
    "where are my bookmarks": "my bookmarks", "bookmark list": "show bookmarks",
    # ---- whatsapp / discord / phone (branch-exact) ----
    "start whatsapp": "open whatsapp", "whatsapp please": "open whatsapp",
    "open the whatsapp": "open whatsapp", "launch the whatsapp": "open whatsapp",
    "open whatsapp web now": "open whatsapp web", "open the web whatsapp": "open whatsapp web",
    "whatsapp on the web": "open whatsapp web",
    "start discord": "open discord", "discord up": "open discord", "open the discord": "open discord",
    "launch the discord": "open discord",
    "find my phone": "ring my phone", "ping my phone": "ring my phone",
    "where is my phone": "ring my phone", "flash my phone": "ring my phone",
    # ---- system info (branch-exact) ----
    "how is my cpu": "cpu usage", "cpu load": "cpu usage", "is my cpu busy": "cpu usage",
    "processor status": "processor",
    "how much ram": "ram usage", "memory left": "memory usage", "how much ram is free": "ram usage",
    "ram left": "ram usage", "memory used": "memory usage",
    "battery percentage": "battery level", "how much battery": "battery",
    "charge status": "battery status", "my battery": "battery",
    "disk space": "disk usage", "free disk": "disk usage", "storage left": "disk usage",
    "drive space": "disk usage", "how much space": "free space",
    "pc specs": "system info", "specs of my pc": "system info",
    "tell me about this computer": "about my pc", "about this machine": "system info",
    "how long has it been on": "uptime", "system uptime": "uptime", "pc uptime": "uptime",
    "what computer am i on": "hostname", "device name": "hostname",
    # ---- misc convenience ----
    "helicopter mode": "screenshot", "photo of the screen": "screenshot",
    "brightness please": "brightness up", "sound check": "volume up",
    "lock it": "lock pc", "silent now": "mute", "loud now": "volume up",
    "quiet now": "volume down", "sleep it": "sleep", "shut it off": "shutdown",
    "shut it down": "shutdown", "turn it off": "shutdown", "start it up": "open",
    "boot it up": "open", "close it down": "close", "kill it": "close",
    "give me a screenshot": "screenshot", "grab a screenshot": "screenshot",
    "shoot the screen": "screenshot", "record screen": "screenshot",
    "make the pc bright": "brightness up", "make the pc dim": "brightness down",
    "crank the brightness": "brightness up", "kill the brightness": "brightness down",
    "restart windows": "restart", "power cycle": "restart", "reset windows": "restart",
    "minimize everything": "minimize all", "clean desktop": "minimize all",
    "show open apps": "open applications", "what apps are open": "open applications",
    "list running apps": "open applications", "open apps": "open applications",
    "running programs": "open applications", "which programs are running": "open applications",
    "whats running": "open applications", "show running apps": "open applications",
    "what is open": "open applications", "which apps": "open applications",
    "apps currently open": "open applications", "show my applications": "open applications",
